...
class GameEngine(object):
    ...
    def update(self, seconds):
        ...
        elif self.kirby.getPosition()[0] + self.kirby.getWidth() > 400:
            self.kirby.position[0] = 400 - self.kirby.getWidth() 
            self.kirby.velocity[0]= 0
        
        if self.kirby.getPosition()[1] <= 0:
            self.kirby.velocity[1] = 0
            self.kirby.position[1] = 0
        elif self.kirby.getPosition()[1] + self.kirby.getHeight() > 200:
            self.kirby.position[1] = 200 - self.kirby.getHeight()
            self.kirby.velocity[1]= 0
